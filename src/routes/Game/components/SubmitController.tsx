...
const SubmitController = ({ gameId, address, wrapper, contract, callback }: Props) => {
    ...
    const submitMove = async () => {
        ...
        try {
            ...
            const tx = await contract.submitHashedMove(gameId, keccak256(hexlify(move)));
            writeMove({ address, gameId, round: wrapper.game.round, move })
            toastInfo('Submit move transaction', 'Submit move transaction was submitted');
            await tx.wait();
