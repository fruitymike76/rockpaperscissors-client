...
const Referral = () => {
    ...
    useEffect(() => {
        ...
        retryPromise(contract.getCurrentContext(), 2)
            .then(context => setReferralRate(context.referralTipRate.toNumber()))
            .catch(error => {
                console.error(error);
            ...
            <Box margin='4px'>
                <StyledBackground>
                    <Flex justifyContent='center' flexDirection='column' padding='8px' borderRadius='8px' maxWidth='480px' margin='auto'>
                        <Text margin={marginText} fontSize='18px' textAlign='left'>To reward people for helping grow this game we've a created referral program.</Text>
                        <Text margin={marginText} fontSize='18px' textAlign='left'>Anyone can register as a referral and share a link.</Text>
                        <Text margin={marginText} fontSize='18px' textAlign='left'>Whenever someone creates and plays out a game using the link referral is going to receive a share of the pot.</Text>
